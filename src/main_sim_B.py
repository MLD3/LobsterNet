...
def main(args):
    ...
    rpath = os.path.join("Synthetic-effect", f"n={args.n}_p={args.p}_amp={args.amplitude}",
        f"type={args.non_compliance_type}", f"effect={args.effect}", f"rep={args.rep}")
    ...
    train_df, test_df = generate_synthetic_nc_effect(
        n=args.n, p=args.p, rep=args.rep, effect=args.effect,
        nc_type=args.non_compliance_type, amplitude=args.amplitude
    )
    ...
    po_act="sigmoid"
    batch_size_list=[128]
    reg_l2_list=[1e-2, 5e-3, 1e-3, 5e-4, 1e-4, 5e-5, 1e-5, 0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--effect", type=float, help="average effect of treatment", default=10)
