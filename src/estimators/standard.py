...
def train_slearner(X_train, T_train, Y_train, checkpoint_dir, overwrite=False, po_act=None, 
                   batch_size=64, verbose=False, early_stop=False, reg_l2=0.01, learning_rate=1e-3, epochs=100):
    
    model = make_slearner(X_train.shape[1], reg_l2=reg_l2, activation=po_act, num_t=1)
...
def train_slearner_grid_search(X_train, T_train, Y_train, checkpoint_dir, overwrite=False, po_act=None, 
...
def train_tarnet(X_train, T_train, Y_train, checkpoint_dir, overwrite=False, po_act=None, 
                 batch_size=64, verbose=False, early_stop=False, reg_l2=0.01, learning_rate=1e-3, epochs=100):
    model = make_tarnet(X_train.shape[1], reg_l2=reg_l2, activation=po_act)
    target_train = np.concatenate([Y_train.reshape(-1, 1), T_train.reshape(-1, 1)], 1)
    input_train = X_train
    if po_act == "sigmoid":
        def loss(a, b): return tlearner_ce_loss(a, b)
...
def train_tarnet_grid_search(X_train, T_train, Y_train, checkpoint_dir, overwrite=False, po_act=None,
        verbose=False, epochs=300, early_stop=True, batch_size_list=[64, 128, 256], reg_l2_list=[1e-2, 1e-3, 1e-4], 
        learning_rate_list=[1e-3, 1e-4]):
    target_train = np.concatenate([Y_train.reshape(-1, 1), T_train.reshape(-1, 1)], 1)
    input_train = X_train
    if po_act == "sigmoid":
